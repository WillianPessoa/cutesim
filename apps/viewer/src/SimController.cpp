...
SimController::SimController(QObject *parent) : QObject(parent), m_client(new SimClient(this)) {
...
QStringList SimController::buildArgs(const QVariantMap &params) {
    ...
    const struct {
        const char *minKey, *maxKey, *modeKey, *durFlag, *modeFlag;
    } devs[] = {
...
void SimController::onSnapshot(const QJsonObject &snap, const QString &raw) {
    ...
    if (!cpuVal.isNull() && !cpuVal.isUndefined()) {
        ...
        if (c.contains("ghost")) {
            m_cpu["ghost"] = c["ghost"].toString();
        }
        if (c.contains("device")) {
            m_cpu["device"] = c["device"].toString();
        }
    ...
    for (const auto &ev : events) {
        ...
        m["pid"]  = e["pid"].toInt();
        if (e.contains("queue")) {
            m["queue"] = e["queue"].toString();
        }
        if (e.contains("device")) {
            m["device"] = e["device"].toString();
        }
        if (e.contains("io_remaining")) {
            m["io_remaining"] = e["io_remaining"].toInt();
        }
        if (e.contains("quantum_used")) {
            m["quantum_used"] = e["quantum_used"].toInt();
        }
        if (e.contains("quantum_max")) {
            m["quantum_max"] = e["quantum_max"].toInt();
        }
        m_events.append(m);
    }
    if (record) {
    ...
    /* ── allProcesses + totalProcessCount ───────────────────────────── */
    {
        ...
        if (!m_cpu.isEmpty()) {
            recordSeen(m_cpu["pid"].toInt());
        }
        for (const QVariant &v : std::as_const(m_highQueue)) {
            recordSeen(v.toMap()["pid"].toInt());
        }
        for (const QVariant &v : std::as_const(m_lowQueue)) {
            recordSeen(v.toMap()["pid"].toInt());
        }
        for (const QVariant &v : std::as_const(m_diskQueue)) {
            recordSeen(v.toMap()["pid"].toInt());
        }
        for (const QVariant &v : std::as_const(m_tapeQueue)) {
            recordSeen(v.toMap()["pid"].toInt());
        }
        for (const QVariant &v : std::as_const(m_printerQueue)) {
            recordSeen(v.toMap()["pid"].toInt());
        }
...
void SimController::clearState() {
    ...
    m_diskQueue.clear();
    m_tapeQueue.clear();
    m_printerQueue.clear();
    m_finished.clear();
    m_stats.clear();
    m_events.clear();
    m_prevEvents.clear();
    m_rawSnapshot.clear();
    m_ganttHistory.clear();
    m_allProcesses.clear();
    m_cpuHistory.clear();
    m_eventsHistory.clear();
    m_rawHistory.clear();
    m_utilHistory.clear();
    m_turnaroundHistory.clear();
...
QVariantList SimController::parseProcessArray(const QJsonArray &arr) {
    ...
    for (const auto &v : arr) {
        ...
        if (o.contains("remaining")) {
            m["remaining"] = o["remaining"].toInt();
        }
        if (o.contains("io_remaining")) {
            m["io_remaining"] = o["io_remaining"].toInt();
        }
        /* finished fields */
        if (o.contains("arrival_tick")) {
            m["arrival_tick"] = o["arrival_tick"].toInt();
        }
        if (o.contains("finish_tick")) {
            m["finish_tick"] = o["finish_tick"].toInt();
        }
        if (o.contains("service_time")) {
            m["service_time"] = o["service_time"].toInt();
        }
        if (o.contains("cpu_time_used")) {
            m["cpu_time_used"] = o["cpu_time_used"].toInt();
        }
        if (o.contains("io_disk")) {
            m["io_disk"] = o["io_disk"].toInt();
        }
        if (o.contains("io_tape")) {
            m["io_tape"] = o["io_tape"].toInt();
        }
        if (o.contains("io_printer")) {
            m["io_printer"] = o["io_printer"].toInt();
        }
        if (o.contains("io_total")) {
            m["io_total"] = o["io_total"].toInt();
        }
        if (o.contains("io_count")) {
            m["io_count"] = o["io_count"].toInt();
        }
        if (o.contains("wait_time")) {
            m["wait_time"] = o["wait_time"].toInt();
        }
        if (o.contains("turnaround")) {
            m["turnaround"] = o["turnaround"].toInt();
        }
        if (o.contains("response_time")) {
            m["response_time"] = o["response_time"].toInt();
        }
        list.append(m);
    }
    return list;
