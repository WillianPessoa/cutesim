...
Item {
    ...
    function openWith(path) {
        if (path && path.length > 0 && bridge) {
            ...
            if (m.ok) {
                filePath = m.path;
                quantumHi = m.quantumHi;
                quantumLo = m.quantumLo;
                seed = m.seed;
                diskMin = m.diskMin;
                diskMax = m.diskMax;
                tapeMin = m.tapeMin;
                tapeMax = m.tapeMax;
                printerMin = m.printerMin;
                printerMax = m.printerMax;
                diskMode = m.diskMode;
                tapeMode = m.tapeMode;
    ...
    function saveTo(path) {
        if (!bridge)
            return false;
        if (!bridge.writeFile(path, toScn()))
            return false;
                        ...
                        delegate: Rectangle {
                            ...
                            RowLayout {
                                ...
                                TextField {
                                    ...
                                    color: Theme.text
                                    placeholderTextColor: Qt.rgba(Theme.textDim.r, Theme.textDim.g,
                                                                  Theme.textDim.b, 0.5)
                                    font.family: "Menlo, Monaco, Courier New, monospace"
                                    font.pixelSize: 10
                                    background: Rectangle {
                                        ...
                                        border.width: 1
                                        border.color: parent.activeFocus ? Theme.accentGlow :
                                                                           Theme.divider
                                    }
                                    onTextEdited: {
                    ...
                    text: {
                        ...
                        return "✓ valid — " + (root.validation.scripted
                                               ? root.validation.processCount + " scripted process"
                                                 ...
                                                 "random workload, " + root.validation.processCount
                                                 + " processes");
                    }
                    color: root.valid ? Theme.accentAlt : root.validation ? Theme.danger :
                                                                            Theme.textDim
