...
GlassCard {
    ...
    function eventLabel(e) {
        if (e.type === "arrived")
            return "arrived → high queue";
        if (e.type === "scheduled")
            return "scheduled ← " + e.queue + " queue";
        if (e.type === "preempted")
            return "preempted " + e.quantum_used + "/" + e.quantum_max + " → low queue";
        if (e.type === "io_start")
            return "→ i/o " + e.device + " (" + e.io_remaining + "t)";
        if (e.type === "io_return")
            return "← i/o " + e.device + " → " + e.queue + " queue";
        if (e.type === "completed")
            return "completed ✓";
        return e.type;
    }
    ...
    function stripColor(state) {
        ...
        if (state === 2)
            return Theme.qLow;
        if (state === 3)
            return Qt.rgba(Theme.qHigh.r, Theme.qHigh.g, Theme.qHigh.b, 0.35);
        if (state === 4)
            return Qt.rgba(Theme.qLow.r, Theme.qLow.g, Theme.qLow.b, 0.35);
        if (state === 5)
            return Theme.qDisk;
        if (state === 6)
            return Theme.qTape;
    ...
    function stripTip(state, tick) {
        var labels = ["", "cpu (high)", "cpu (low)", "high queue", "low queue", "i/o disk",
                      "i/o tape", "i/o printer"];
        ...
        ColumnLayout {
            ...
            Flickable {
                ...
                Row {
                    id: stripRow
                    spacing: 1
                    readonly property int blockW: Math.max(8, Math.floor((stripFlick.width - (
                                                                              root.stripData.length
                                                                              - 1)) / Math.max(1,
                                                                                               root.stripData.length)))
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            opacity: modelData === 0 ? 0 : (modelData >= 3 && modelData <= 4)
                                                       ? 0.55 : 0.88
                            HoverHandler {
                                id: blkHov
                            }
            ...
            Flow {
                ...
                Repeater {
                    model: [
                        ...
                        {
                            color: Qt.rgba(Theme.qHigh.r, Theme.qHigh.g, Theme.qHigh.b, 0.35),
                            label: "queued"
                        },
                        {
                            color: Theme.qDisk,
