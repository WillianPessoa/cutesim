...
Window {
    ...
    readonly property var selectedProcess: {
        if (selectedPid < 0 || !controller)
            return null;
        var list = controller.allProcesses;
        for (var i = 0; i < list.length; i++)
            if (list[i].pid === selectedPid)
                return list[i];
        return null;
    }
    ...
    readonly property var newlyQueued: {
        ...
        for (var j = 0; j < prev.length; j++) {
            if (prev[j].type === "preempted")
                r.low.push(prev[j].pid);
            if (prev[j].type === "io_start") {
                if (prev[j].device === "disk")
                    r.disk.push(prev[j].pid);
                else if (prev[j].device === "tape")
                    r.tape.push(prev[j].pid);
                else if (prev[j].device === "printer")
                    r.printer.push(prev[j].pid);
            }
        }
        return r;
    }
    ...
    readonly property string cpuGhost: {
        if (!controller || !controller.cpu || controller.cpu.pid === undefined)
            return "";
        var g = controller.cpu.ghost;
        if (g === "io_start")
            return "io_" + controller.cpu.device;
        return g || "";
    }
    ...
    ColumnLayout {
        ...
        Header {
            ...
            status: controller && controller.done ? "done" : controller && controller.connected
                                                    ? "connected" : "disconnected"
            connected: controller ? controller.connected : false
            simDone: controller ? controller.done : false
            onStepRequested: if (controller)
                                 controller.step()
            onResetRequested: if (controller)
                                  controller.reset()
            onReconfigureRequested: if (controller)
    ...
    LaunchOverlay {
        ...
        bridge: (typeof scenarioBridge !== "undefined") ? scenarioBridge : null
        onLaunchRequested: params => {
                               if (controller)
                               controller.launch(params);
                           }
    ...
    Connections {
        ...
        function onLaunchStateChanged() {
            if (!controller || !controller.needsLaunch)
                return;
    ...
    Connections {
        ...
        function onErrorOccurred(msg) {
            toast.show(msg, Theme.danger);
        }
        function onInfoOccurred(msg) {
            toast.show(msg, Theme.accentAlt);
        }
    }
